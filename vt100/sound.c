...
static void speaker (void)
{
  int i;
  //number of bytes
  //logger ("SND", "Buffer %u", SDL_GetQueuedAudioSize (dev));
  if (SDL_GetQueuedAudioSize (dev) > 480)
    SDL_Delay (10);
...
void reset_sound (void)
{
  ...
  if (have.format != want.format)
    logger ("SND", "Didn't get wanted format.");
  logger ("SND", "Frequency: %d, channels: %d", have.freq, have.channels);
  speaker ();
