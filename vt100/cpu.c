...
static SDL_SpinLock lock_cycles;
...
static SDL_atomic_t irq;

unsigned long long get_cycles (void)
{
  unsigned long long x;
  SDL_AtomicLock (&lock_cycles);
  x = cycles;
  SDL_AtomicUnlock (&lock_cycles);
  return x;
}
...
#if 0
  if (addr >= 0x2E00)
    logger ("CPU", "Load from %04X; PC %04X", addr, reg_PC);
#endif
  return memory[addr];
}
...
#if 0
  if (addr >= 0x2E00)
    logger ("CPU", "Store %02X to %04X; PC %04X", data, addr, reg_PC);
#endif
  if (addr == 0x2000)
    logger ("CPU", "Storing %02X to 2000; PC is now %04X", data, reg_PC);
  if (mtype[addr >> 10] == 0)
    memory[addr] = data;
}
