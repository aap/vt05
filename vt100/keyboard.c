...
static void keyboard_out (u8 port, u8 data)
{
  ...
  previous = data;
  if (data & 0x40) {
    if (scan_event.cycles > 0 && scan < 0x80)
      logger ("KEY", "Scan aborted at %02X", scan);
    scan = 0;
    scanning ();
  }
  if (data & 0x80)
...
{
  logger ("KEY", "Down %02X", code);
  down[code] = 1;
}

void key_up (u8 code)
{
  logger ("KEY", "Up %02X", code);
  down[code] = 0;
}
